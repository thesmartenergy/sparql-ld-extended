...
import static org.apache.jena.atlas.io.NullOutputStream.sink;
...
public class ReadRDFFromIRI {
    ...
    private void read() {
        ...
        if (iri.toLowerCase().endsWith(".ntriples") || iri.toLowerCase().endsWith(".nt")) {
            ...
            qe = (QueryExecutionBase) QueryExecutionFactory.create(query, model);
            resultSet = qe.execSelect();
        } else if (iri.toLowerCase().endsWith(".ttl")) {
            System.out.println("# Trying to read a 'turtle' file...");
            model.read(iri);
            qe = (QueryExecutionBase) QueryExecutionFactory.create(query, model);
            resultSet = qe.execSelect();
        } else {
            ...
            /*if (contentType.contains("text/html") || contentType.contains("application/xhtml+xml")) {
                ...
                System.out.println("Cannot read RDFa");
